...
export default {
  ...
  async mounted() {
    console.clear()
    const encryptedAddress = decodeURIComponent(this.$route.query.file)

    const decryptedAddress = crypto.AES.decrypt(encryptedAddress, process.env.VUE_APP_SECRET_KEY).toString(crypto.enc.Utf8)
    console.log(decryptedAddress)

    try {
      this.file = await systemAPIService.getItemByAddress(decryptedAddress, this.$router)
    } catch (e) {
